...
from BSpline.Interface.GLStandardDrawingWindow import *
            ...
            if event.buttons() == QtCore.Qt.LeftButton:
                ...
                for curve in self.history:
                    for point in self.userDefinedPoints:
                        if (event.x() - bounds < point.x < event.x() + bounds
                                and self.height - event.y() + bounds > point.y > self.height - event.y() - bounds):
                            curve.compute(curve.controlPoints)
                            point.translate(lastMousePosition.x() - point.x,
                                            self.height - lastMousePosition.y() - point.y, 0)
                            self.updateGL()
